...
<head>
  ...
  <title>Fix&amp;Go — Revenue Report</title>
  ...
  <style>
    ...
    .range-tabs { display: flex; gap: 0.5rem; flex-wrap: wrap; }
    .range-tab { padding: 0.4rem 1.1rem; border-radius: 20px; border: 1.5px solid var(--fg-border); background: transparent; color: var(--fg-muted); font-size: 0.82rem; font-weight: 600; cursor: pointer; transition: all 0.2s ease; }
    .range-tab:hover { border-color: var(--fg-primary); color: var(--fg-primary); }
    .range-tab.active { background: var(--fg-primary); border-color: var(--fg-primary); color: #fff; }
    .kpi-card { background: var(--fg-card-bg); border-radius: var(--fg-radius); border: 1px solid var(--fg-border); padding: 1.25rem 1.5rem; transition: transform 0.2s ease, box-shadow 0.2s ease; }
    ...
    .report-table th { background: var(--fg-primary); color: #fff; padding: 0.65rem 0.9rem; text-align: left; font-weight: 700; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.5px; white-space: nowrap; }
    .report-table td { padding: 0.7rem 0.9rem; border-bottom: 1px solid var(--fg-border); color: var(--fg-text); vertical-align: middle; }
    ...
    .pct-bar { height: 6px; border-radius: 3px; background: var(--fg-border); overflow: hidden; margin-top: 4px; }
    .pct-fill { height: 100%; border-radius: 3px; background: var(--fg-primary); }
    .section-title { font-size: 1rem; font-weight: 700; color: var(--fg-text); margin-bottom: 1rem; }
...
<body>
  ...
  <div class="supplier-layout">
    ...
    <aside class="supplier-sidebar" id="supplierSidebar">
      ...
      <ul class="sidebar-nav">
        ...
        <li><a href="sales-report.php" class="active"><i class="bi bi-bar-chart-line"></i> Revenue Report</a></li>
    ...
    <main class="supplier-main">
      ...
      <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4">
        <div>
          <h2 style="font-weight:800;color:var(--fg-text);margin:0;">Revenue Report</h2>
          <p style="color:var(--fg-muted);margin:0;font-size:0.9rem;">Shop earnings and performance overview</p>
        </div>
      ...
      <div class="row g-3 mb-4">
        <div class="col-6 col-lg-3">
          <div class="kpi-card">
            <div class="kpi-icon" style="background:rgba(16,185,129,0.12);color:#059669;"><i class="bi bi-currency-exchange"></i></div>
            <div class="kpi-value" style="color:#059669;" id="kpiRevenue">₱140,300</div>
            <div class="kpi-label">Total Revenue</div>
          </div>
        </div>
        <div class="col-6 col-lg-3">
          <div class="kpi-card">
            <div class="kpi-icon" style="background:rgba(59,130,246,0.12);color:#2563eb;"><i class="bi bi-bag-check"></i></div>
            <div class="kpi-value" style="color:#2563eb;" id="kpiOrders">131</div>
            <div class="kpi-label">Bookings Completed</div>
          </div>
        </div>
        <div class="col-6 col-lg-3">
          <div class="kpi-card">
            <div class="kpi-icon" style="background:rgba(139,92,246,0.12);color:#7c3aed;"><i class="bi bi-calculator"></i></div>
            <div class="kpi-value" style="color:#7c3aed;" id="kpiAvg">₱1,071</div>
            <div class="kpi-label">Avg Booking Value</div>
          </div>
        </div>
        <div class="col-6 col-lg-3">
          <div class="kpi-card">
            <div class="kpi-icon" style="background:rgba(230,168,0,0.12);color:#c98f00;"><i class="bi bi-trophy"></i></div>
            <div class="kpi-value" style="color:#c98f00;font-size:1.1rem;" id="kpiTop">Screen Replacement</div>
  ...
  <script>
    const DATA = {
      week: {
        revenue: '₱32,500', orders: 28, avg: '₱1,161', top: 'Screen Replacement',
        products: [
          { rank:1, name:'Screen Replacement',  cat:'Hardware', units:12, rev:'₱18,000', pct:55 },
          { rank:2, name:'Battery Replacement', cat:'Hardware', units:9,  rev:'₱7,200',  pct:22 },
          { rank:3, name:'Water Damage Repair', cat:'Hardware', units:3,  rev:'₱6,000',  pct:18 },
          { rank:4, name:'Charging Port Fix',   cat:'Hardware', units:6,  rev:'₱3,600',  pct:11 },
          { rank:5, name:'Speaker/Mic Repair',  cat:'Hardware', units:2,  rev:'₱1,400',  pct:4  },
        ],
      },
      month: {
        revenue: '₱140,300', orders: 131, avg: '₱1,071', top: 'Screen Replacement',
        products: [
          { rank:1, name:'Screen Replacement',  cat:'Hardware', units:45, rev:'₱67,500', pct:48 },
          { rank:2, name:'Battery Replacement', cat:'Hardware', units:38, rev:'₱30,400', pct:22 },
          { rank:3, name:'Water Damage Repair', cat:'Hardware', units:12, rev:'₱24,000', pct:17 },
          { rank:4, name:'Charging Port Fix',   cat:'Hardware', units:28, rev:'₱16,800', pct:12 },
          { rank:5, name:'Speaker/Mic Repair',  cat:'Hardware', units:8,  rev:'₱5,600',  pct:4  },
        ],
      },
      year: {
        revenue: '₱1,683,600', orders: 1572, avg: '₱1,071', top: 'Screen Replacement',
        products: [
          { rank:1, name:'Screen Replacement',  cat:'Hardware', units:540, rev:'₱810,000', pct:48 },
          { rank:2, name:'Battery Replacement', cat:'Hardware', units:456, rev:'₱364,800', pct:22 },
          { rank:3, name:'Water Damage Repair', cat:'Hardware', units:144, rev:'₱288,000', pct:17 },
          { rank:4, name:'Charging Port Fix',   cat:'Hardware', units:336, rev:'₱201,600', pct:12 },
          { rank:5, name:'Speaker/Mic Repair',  cat:'Hardware', units:96,  rev:'₱67,200',  pct:4  },
        ],
      },
    };

    const MONTHLY = [
      { month:'January',  orders:98,  revenue:'₱104,958', growth:'+8%',  pos:true  },
      { month:'February', orders:82,  revenue:'₱87,822',  growth:'-5%',  pos:false },
      { month:'March',    orders:119, revenue:'₱127,449', growth:'+45%', pos:true  },
      { month:'April',    orders:142, revenue:'₱152,082', growth:'+19%', pos:true  },
      { month:'May',      orders:126, revenue:'₱134,946', growth:'-11%', pos:false },
      { month:'June',     orders:131, revenue:'₱140,300', growth:'+4%',  pos:true  },
    ];
