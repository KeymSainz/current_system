...
<head>
  ...
  <title>Fix&amp;Go â€” Inventory</title>
  ...
  <style>
    ...
    .badge-display { background: rgba(59,130,246,0.12); color: #3b82f6; }
    ...
    .product-img { width: 48px; height: 48px; border-radius: 8px; object-fit: cover; background: var(--fg-bg); }
    .product-img-ph { width: 48px; height: 48px; border-radius: 8px; background: var(--fg-bg); border: 1px solid var(--fg-border); display: flex; align-items: center; justify-content: center; color: var(--fg-muted); font-size: 1.1rem; }
    .btn-display { display: inline-flex; align-items: center; gap: 0.3rem; padding: 0.35rem 0.85rem; border-radius: 8px; font-size: 0.78rem; font-weight: 700; cursor: pointer; border: 1.5px solid; transition: all 0.2s; white-space: nowrap; }
    .btn-display.add    { border-color: #28A745; color: #28A745; background: rgba(40,167,69,0.08); }
    .btn-display.add:hover    { background: #28A745; color: #fff; }
    .btn-display.remove { border-color: #dc3545; color: #dc3545; background: rgba(220,53,69,0.08); }
    .btn-display.remove:hover { background: #dc3545; color: #fff; }
...
<body>
  ...
  <div class="sp-layout">
    <aside class="sp-sidebar" id="spSidebar">
      ...
      <ul class="sidebar-nav">
        ...
        <li><a href="products.php"><i class="bi bi-shop"></i> Manage Products</a></li>
    ...
    <main class="sp-main">
      <div class="page-header">
        <div>
          <h2><i class="bi bi-clipboard-data" style="color:#3b82f6;margin-right:0.5rem;"></i>Inventory</h2>
          <p>Products sent by the supervisor â€” choose which ones to display to customers</p>
        </div>
        <a href="products.php" class="btn-primary-custom" style="text-decoration:none;">
          <i class="bi bi-shop"></i> View Displayed Products
        </a>
      </div>
      ...
      <div class="alert-bar alert-info">
        ...
        Click <strong>Add to Display</strong> to make a product visible to customers on the shop. Click <strong>Remove</strong> to hide it.
      </div>

      <div class="stats-row">
        <div class="stat-card">
          <div class="stat-icon" style="background:rgba(59,130,246,0.12);color:#3b82f6;"><i class="bi bi-clipboard-data"></i></div>
          <div><div class="stat-value" style="color:#3b82f6;" id="statTotal">0</div><div class="stat-label">Total in Inventory</div></div>
        </div>
        <div class="stat-card">
      ...
      <div class="toolbar">
        <div style="display:flex;gap:0.5rem;flex-wrap:wrap;">
          ...
          <button class="btn-filter" data-filter="displayed">Displayed</button>
          <button class="btn-filter" data-filter="hidden">Not Displayed</button>
          ...
          <table class="data-table">
            ...
            <tbody id="inventoryTableBody">
              <tr><td colspan="9"><div class="empty-state">
                <div style="width:28px;height:28px;border:3px solid var(--fg-border);border-top-color:#3b82f6;border-radius:50%;animation:spin 0.7s linear infinite;margin:0 auto 0.75rem;"></div>
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadInventory() {
      fetch('../../../backend/sales_inventory.php?action=list')
        .then(r => r.json())
        .then(d => {
        ...
        .catch(() => {
          document.getElementById('inventoryTableBody').innerHTML =
            '<tr><td colspan="9"><div class="empty-state"><i class="bi bi-inbox"></i><p>No products in inventory yet. The supervisor has not sent any products.</p></div></td></tr>';
        });
    }

    function renderStats() {
      document.getElementById('statTotal').textContent     = allItems.length;
      document.getElementById('statDisplayed').textContent = allItems.filter(i => i.is_displayed == 1).length;
    ...
    function renderTable() {
      ...
      if (currentFilter === 'displayed') filtered = filtered.filter(i => i.is_displayed == 1);
      if (currentFilter === 'hidden')    filtered = filtered.filter(i => i.is_displayed == 0);
      ...
      if (!filtered.length) {
        tbody.innerHTML = '<tr><td colspan="9"><div class="empty-state"><i class="bi bi-inbox"></i><p>No items found.</p></div></td></tr>';
        return;
      }
      ...
      tbody.innerHTML = filtered.map(item => {
        ...
        const isDisplayed = item.is_displayed == 1;
        const displayBadge = isDisplayed
          ? '<span class="badge-status badge-display"><i class="bi bi-eye-fill" style="margin-right:0.25rem;"></i>Displayed</span>'
          : '<span class="badge-status badge-hidden"><i class="bi bi-eye-slash" style="margin-right:0.25rem;"></i>Hidden</span>';

        const toggleBtn = isDisplayed
          ? `<button class="btn-display remove" onclick="toggleDisplay(${item.id})"><i class="bi bi-eye-slash-fill"></i> Remove</button>`
          : `<button class="btn-display add" onclick="toggleDisplay(${item.id})"><i class="bi bi-eye-fill"></i> Add to Display</button>`;

        const requestBtn = `<a href="supply-requests.php?product=${encodeURIComponent(item.name)}&category=${encodeURIComponent(item.category || '')}"
          style="display:inline-flex;align-items:center;gap:0.3rem;padding:0.3rem 0.65rem;border-radius:8px;background:rgba(230,168,0,0.08);color:var(--fg-primary);border:1.5px solid rgba(230,168,0,0.25);font-size:0.75rem;font-weight:700;text-decoration:none;margin-left:0.35rem;"
          title="Request more stock"><i class="bi bi-send-fill"></i></a>`;
        ...
        return `<tr id="inv-row-${item.id}">
          ...
          <td>${displayBadge}</td>
          <td style="text-align:center;white-space:nowrap;">${toggleBtn}${requestBtn}</td>
        </tr>`;
      }).join('');
    }

    // Toggle display on/off
    window.toggleDisplay = function(productId) {
      fetch('../../../backend/sales_inventory.php', {
        method: 'POST',
